...
class AlwaysAuthenticatedMiddlewaree(MiddlewareMixin):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (not settings.DEBUG and
            ...
            raise ImproperlyConfigured(
                'DEBUG=%s, but AlwaysAuthenticatedMiddlewaree is configured to '
                'only run in debug mode.\n'
                'Remove AlwaysAuthenticatedMiddlewaree from '
                'MIDDLEWARE/MIDDLEWARE_CLASSES or set '
                'ALWAYS_AUTHENTICATED_DEBUG_ONLY to False.' % settings.DEBUG)
        super(AlwaysAuthenticatedMiddlewaree, self).__init__(*args, **kwargs)
